    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3 text-center">
          ...
          <p className="text-xs font-bold text-emerald-800 mt-0.5">{formatRp(monthlyIncome)}</p>
        </div>
        <div className="bg-red-50 border border-red-100 rounded-2xl p-3 text-center">
          <TrendingDown className="w-5 h-5 text-red-500 mx-auto mb-1" />
          <p className="text-xs text-red-600 font-medium">Keluar</p>
          <p className="text-xs font-bold text-red-800 mt-0.5">{formatRp(monthlyExpense)}</p>
        </div>
        <div className={`border rounded-2xl p-3 text-center ${monthlySaving >= 0 ? "bg-blue-50 border-blue-100" : "bg-orange-50 border-orange-100"}`}>
          ...
          <p className={`text-xs font-bold mt-0.5 ${monthlySaving >= 0 ? "text-blue-800" : "text-orange-800"}`}>{formatRp(Math.abs(monthlySaving))}</p>
        </div>
      </div>
        ...
        {transactions.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={12}>
              ...
              <YAxis tick={{ fontSize: 10 }} tickFormatter={(v) => formatRp(v)} />
