...
export default function AddTransactionModal({ isOpen, onClose, defaultType = "expense", prefillDescription = "", prefillAmount, defaultSource = "manual", editTransaction }: Props) {
...
function AddTransactionForm({ onClose, defaultType = "expense", prefillDescription = "", prefillAmount, defaultSource = "manual", editTransaction }: Omit<Props, "isOpen">) {
  ...
  const [category, setCategory] = useState(editTransaction?.category || (type === "income" ? "Gaji" : "Makanan"));
  const [description, setDescription] = useState(editTransaction?.description || prefillDescription);
  const [date, setDate] = useState(editTransaction ? new Date(editTransaction.date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEdit && editTransaction) {
      ...
      addNotification("Transaksi berhasil diperbarui! ✏️", "success");
    } else {
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-md p-6 shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-gray-900">
            {isEdit ? "Edit Transaksi" : type === "income" ? "Tambah Pemasukan" : "Tambah Pengeluaran"}
          </h2>
