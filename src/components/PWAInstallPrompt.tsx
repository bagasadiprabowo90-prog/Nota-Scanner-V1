      ...
      <div className="fixed bottom-20 left-4 right-4 z-50 max-w-lg mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl border border-emerald-100 p-4 flex items-start gap-3 animate-slide-up">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-gray-900 text-sm">Install Money Scanner</p>
          ...
          <div className="bg-white rounded-t-2xl w-full max-w-lg p-6 pb-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              {[
                ...
                { step: "4", text: 'Aplikasi Money Scanner akan muncul di Home Screen! 🎉' },
              ].map(({ step, text }) => (
                <div key={step} className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-full bg-emerald-500 text-white text-sm font-bold flex items-center justify-center shrink-0">{step}</div>
                  <p className="text-sm text-gray-700 pt-1" dangerouslySetInnerHTML={{ __html: text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
                </div>
              ))}
            </div>
            <button
              onClick={() => { setShowIOSGuide(false); setShowBanner(false); sessionStorage.setItem("pwa_dismissed", "1"); }}
